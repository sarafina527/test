...
{
    // int[][][][] dp = new int[boxTemplate.length+1][boxTemplate.width+1][boxTemplate.height][CUSTOMS_LIMIT_MONEY_PER_BOX];
    // boolean hasPut[] = new boolean[items];
    int sumprice = 0;
    for(Model m:items){
        sumprice+=m.price;
    }
    int ans = sumprice/2000;
    return ans;
}
static Model[] items;
